...
if [ $2 -eq 1 ]; then
	...
	curl -X POST -H 'Content-type: application/json' --data '{"text":"Run Subdomain Eumeration _*`Completed`*_  - '$DOMAIN'"}' $SLACK
elif [ $2 -eq 2 ]; then
	./httprobe.sh $SUBDOMAIN_PATH $DOMAIN_PATH
	curl -X POST -H 'Content-type: application/json' --data '{"text":"Run HTTPROBE _*`Completed`*_  - '$DOMAIN'"}' $SLACK
elif [ $2 -eq 3 ]; then
	read -p "ENTER MODE (CRITICAL,HIGH,MEDIUM,LOW,INFO,ALL): " MODE
	./nuclei.sh $SERVER_PATH $DOMAIN_PATH $MODE
	curl -X POST -H 'Content-type: application/json' --data '{"text":"Run NUCLEI '$MODE' _*`Completed`*_  - '$DOMAIN'"}' $SLACK
elif [ $2 -eq 4 ]; then
